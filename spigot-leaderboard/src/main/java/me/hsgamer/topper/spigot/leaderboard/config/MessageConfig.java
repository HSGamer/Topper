package me.hsgamer.topper.spigot.leaderboard.config;
...
public class MessageConfig extends PathableConfig {
    ...
    public static final SimpleConfigPath<List<String>> SIGN_LINES = new SimpleConfigPath<List<String>>("sign-lines", Arrays.asList(
        ...
        public void migrateConfig(@NotNull Config config) {
            JavaPlugin plugin = JavaPlugin.getPlugin(TopperLeaderboard.class);
            File file = new File(plugin.getDataFolder(), "sign.yml");
            if (file.exists()) {
                YamlConfiguration yamlConfiguration = YamlConfiguration.loadConfiguration(file);
                if (yamlConfiguration.contains(getPath())) {
                    config.set(getPath(), yamlConfiguration.get(getPath()));
                    yamlConfiguration.set(getPath(), null);
                    try {
